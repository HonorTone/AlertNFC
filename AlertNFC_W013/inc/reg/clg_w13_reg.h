 ...
 *
 * @file	clg_w13_reg.h
 * @brief	Definition of the Clock Generator(CLG) Register for S1C17W13.
 *
 * @par		Copyright:
...
typedef union CLGOSC1_REG_TAG {
	...
	struct CLGOSC1_REG_BIT_TAG {
		...
		unsigned short		OSC1SELCR	: 1;	///< This bit selects an oscillator type of the OSC1 oscillator circuit..
...
typedef union CLGTRIM_REG_TAG {
	unsigned short			reg;
	struct CLGTRIM_REG_BIT_TAG {
		unsigned short		IOSCAJ		: 6;	///< These bits sets the frequency trimming value for the IOSC internal oscillator circuit.
		unsigned short		reserved_1	: 2;	///< Reserved.
		unsigned short		OSC3AJ		: 5;	///< These bits sets the frequency trimming value for the OSC3 internal oscillator circuit.
